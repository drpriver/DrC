    ...
    if(sv_equals(tok.txt, SV("define")) || sv_equals(tok.txt, SV("defifndef"))){
        ...
        if(tok.type == CPP_NEWLINE || tok.type == CPP_EOF){
            ...
            for(;;){
                ...
                // GCC extension: name... (named varargs)
                {
                    CppToken peek;
        ...
        for(;;){
            ...
            if(tok.type == CPP_WHITESPACE && at_beginning_of_line)
                continue;
            if(at_beginning_of_line && tok.type == CPP_PUNCTUATOR && tok.punct == '#'){
                CppToken dir;
                do {
        ...
        if(tok.type == CPP_NEWLINE || tok.type == CPP_EOF){ // bare #pragma
            return cpp_push_tok(cpp, &cpp->pending, tok);
        }
        if(tok.type != CPP_IDENTIFIER){
            do {
        ...
        // EXTENSION: try each header candidate on the line, picking the first one.
        for(;;){
            if(tok.type == CPP_NEWLINE || tok.type == CPP_EOF){
                err = cpp_push_tok(cpp, &cpp->pending, tok);
                if(err) goto cleanup;
...
cpp_expand_obj_macro(CppPreprocessor *cpp, CppMacro *macro, SrcLoc expansion_loc, CppTokens *dst){
    ...
    for(size_t i = 0; i < macro->nreplace; i++){
        if(repl[i].type == CPP_PUNCTUATOR && repl[i].punct == '##'){
            ...
            for(size_t j = result->count; j--;){
                CppToken tok = result->data[j];
...
cpp_define_target_macros(CppPreprocessor* cpp){
    ...
    DEFINT("__DVM_ATOMIC_LOCK_FREE_MAX", t.atomic_lock_free_max);
...
cpp_setup_builtin_headers(CppPreprocessor* cpp){
    ...
    static const struct { StringView name; StringView content; } headers[] = {
        ...
        {SVI("stdatomic.h"), SVI("#pragma once\n"
                              ...
                              "#include <__stdatomic_typedefs.h>\n"
                              "#define ATOMIC_VAR_INIT(value) (value)\n"
                              "#define atomic_init(obj, ...) ((void)(*obj = __VA_ARGS__))\n"
                              "typedef _Atomic bool atomic_flag;\n"
                              "#define ATOMIC_FLAG_INIT 0\n"
                              "#include <__stdatomic_ops.h>\n"
                           )},
        {SVI("__stdatomic_ops.h"), SVI("#pragma once\n"
                              "#define atomic_is_lock_free(x) ((sizeof(*(x)) == 1 || sizeof(*(x)) == 2 || sizeof(*(x)) == 4 || sizeof(*(x)) == 8 || sizeof(*(x)) == 16) && sizeof(*(x)) <= __DVM_ATOMIC_LOCK_FREE_MAX)\n"
                              ...
                              "#define atomic_compare_exchange_strong(obj, expected, desired) __atomic_compare_exchange_n(obj, expected, desired, 0, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)\n"
                              "#define atomic_compare_exchange_strong_explicit(obj, expected, desired, s, f) __atomic_compare_exchange_n(obj, expected, desired, 0, s, f)\n"
                              "#define atomic_compare_exchange_weak(obj, expected, desired) __atomic_compare_exchange_n(obj, expected, desired, 1, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)\n"
                              "#define atomic_compare_exchange_weak_explicit(obj, expected, desired, s, f) __atomic_compare_exchange_n(obj, expected, desired, 1, s, f)\n"
        ...
        {SVI("__stdatomic_typedefs.h"), SVI("#pragma once\n"
